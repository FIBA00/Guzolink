...
const queue = previewShops.map((shop, index) => ({
  ...
  merchantEmail: [
    "mekdes@guzolink.local",
    "amanuel@guzolink.local",
    "hana@guzolink.local",
  ][index],
  ...
  persist(
    set => ({
      ...
      decide: (id, decision, reason = "") =>
        set(state => ({
          storefronts: state.storefronts.map(shop =>
            shop.id === id
              ? {
                  ...shop,
      ...
      sendResubmissionReminder: id =>
        set(state => ({
          resubmissions: state.resubmissions.map(item =>
            item.id === id ? { ...item, status: "Reminder sent" } : item
          ),
        })),
    }),
