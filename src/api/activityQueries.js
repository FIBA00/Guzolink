...
export function useActivities(params = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      if (isPreviewMode())
        return {
          items: useNotificationStore.getState().activities,
          total: useNotificationStore.getState().activities.length,
        };
      const result = await activitiesApi.list(params);
      const items = Array.isArray(result)
        ? result
        : result?.items || result?.activities || result?.results || [];
...
export function useCreateActivity() {
  ...
  return useMutation({
    mutationFn: async activity =>
      isPreviewMode()
        ? useNotificationStore.getState().addActivity(activity)
        : activitiesApi.create(activity),
    onSuccess: () => client.invalidateQueries({ queryKey: activityKey }),
  });
}
  ...
  return useMutation({
    mutationFn: async id =>
      isPreviewMode()
        ...
        : activitiesApi.update(id, { read: true }),
    onSuccess: () => client.invalidateQueries({ queryKey: activityKey }),
  });
}
export function useClearReadActivities() {
  const client = useQueryClient();
  return useMutation({
    mutationFn: async () =>
      isPreviewMode()
        ? useNotificationStore.getState().clearRead()
        : activitiesApi.update("read", { action: "clear" }),
    onSuccess: () => client.invalidateQueries({ queryKey: activityKey }),
  });
}
