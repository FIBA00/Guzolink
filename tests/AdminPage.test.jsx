...
const reviewQueue = previewShops.map(shop => ({
  ...shop,
  merchantName: "Demo merchant",
  merchantEmail: "merchant@guzolink.local",
...
describe("AdminPage review queue", () => {
  beforeEach(() => {
    useAdminStore.setState({
      storefronts: reviewQueue.map(shop => ({ ...shop })),
    });
    useNotificationStore.setState({ activities: [] });
  });
  it("allows an administrator to approve a pending storefront and records the decision", async () => {
    ...
    fireEvent.click(
      await screen
        .findAllByRole("button", { name: "Review" })
        .then(buttons => buttons[0])
    );
    fireEvent.click(screen.getByRole("button", { name: "Approve storefront" }));
    await waitFor(() =>
      expect(useAdminStore.getState().storefronts[0].approvalStatus).toBe(
        "approved"
      )
    );
