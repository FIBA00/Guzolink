...
describe("MarketplaceShell navigation", () => {
  ...
  it("opens and closes the accessible sidebar from the navbar menu button", () => {
    renderShell();
    const menuButton = screen.getByRole("button", { name: "Menu" });
    fireEvent.click(menuButton);
    const sidebar = screen.getByRole("dialog", { name: "Menu" });
    expect(sidebar.className).toContain("translate-x-0");
    fireEvent.click(
      screen.getAllByRole("button", { name: "Close menu" }).at(-1)
    );
    expect(sidebar.className).toContain("translate-x-full");
  });
  it("switches shared navigation labels to Amharic", () => {
    renderShell();
    const toggles = screen.getAllByRole("button", {
      name: "English / Amharic",
    });
    fireEvent.click(toggles[0]);
    expect(screen.getAllByText("ይመልከቱ").length).toBeGreaterThan(0);
  });
});
